        ...
        while(true){
            try{
                ...
                if("OK".equals(ok)){
                    boolean bool = true;
                    while(bool){
                        System.out.println("开始执行失败队列");
                        List<String> taskList = this.getTasks();
                        List<String> failureTaskList = new ArrayList<String>();
                        ...
                        System.out.println(String.format("失败队列执行完毕:共执行%d个任务,其中失败了%d个任务", taskList.size(), failureTaskListSize));
                        bool = false;
                    }
                }else{
                    sleep(1000);
